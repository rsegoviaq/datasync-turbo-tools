...
╔═══════════════════════════════════════════════════════════════╗
║                                                               ║
║         DataSync Turbo Tools - Quick Install v1.2.2          ║
║                                                               ║
...
check_prerequisites() {
    ...
    # Check Bash version
    # macOS ships with bash 3.2 due to licensing, which is sufficient for our needs
    ...
    if [[ "${BASH_VERSINFO[0]}" -lt "$min_bash_version" ]]; then
        log_error "Bash ${min_bash_version}.0 or higher is required (found: $BASH_VERSION)"
        missing_deps+=("bash >= ${min_bash_version}.0")
    else
...
configure_aws_profile() {
    ...
    while true; do
        ...
        if aws sts get-caller-identity --profile "$AWS_PROFILE_INPUT" &>/dev/null; then
            ...
            # Get region from profile (required for s5cmd)
            AWS_REGION_VALUE=$(aws configure get region --profile "$AWS_PROFILE_INPUT" 2>/dev/null || echo "us-east-1")
            log_info "Detected region: $AWS_REGION_VALUE"
...
configure_s3() {
    ...
    while true; do
        ...
        # s5cmd requires AWS_REGION to be set as environment variable
...
write_configuration() {
...

    {
        ...
        echo "# DataSync Turbo Tools v1.3.1"
        echo
        ...
        if [[ "$AWS_AUTH_METHOD" == "profile" ]]; then
            ...
            echo "AWS_REGION=\"$AWS_REGION_VALUE\"  # Required for s5cmd"
